...
Drop columns that we do not need in the correlation matrix
...
Create the matrix from the dataframe
...
```{r}
ggcorrplot(breast_cancer_correlations, method = "square", type = "lower", lab = TRUE, lab_size = 1)
```
