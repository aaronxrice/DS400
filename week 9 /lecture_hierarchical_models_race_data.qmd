...
```{r}
mean_x <- mean(running$net, na.rm = TRUE)
mean_y <- mean(running$age, na.rm = TRUE)

ggplot(running, aes(x = net, y = age)) +
  geom_point() +
  scale_x_continuous(breaks = seq(60, 140, by = 5)) + 
  geom_vline(xintercept = mean_x, linetype = "dashed", color = "blue") +  # Vertical line at mean of x
  geom_hline(yintercept = mean_y, linetype = "dashed", color = "blue") +  # Horizontal line at mean of y
  labs(title = "Running Times and Age of Runners", 
       x = "Time", 
       y = "Age") +
  theme_minimal()
```
