...
```{r}
p_a <- mean(patients$test_result == "positive")

p_b <- n_diseased / n_patients

p_a_given_b <- mean(patients$test_result[patients$has_disease == 1] == "positive")
...
The probability someone has the disease given two positive tests\
P(B\|A1\*A2) Calculate overall probability of two positive tests (A1\*A2) P(A) for two positive test results\
Calculate P(B\|A) using Bayes' theorem for two positive tests

```{r}
p_a_given_b_two_positive <- sensitivity * sensitivity

p_b <- n_diseased / n_patients

p_a_two_positive <- (sensitivity^2 * p_b) + 
                    (false_positive_rate^2 * (1 - p_b))

p_b_given_a_two_positive <- (p_a_given_b_two_positive * p_b) / p_a_two_positive

cat("P(B|A for two positives):", p_b_given_a_two_positive, "\n")
```

The likelihood of having the disease significantly increases given a second positive test
