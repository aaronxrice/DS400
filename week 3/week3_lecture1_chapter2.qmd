...
## need to see if pop column is true
...
-   Letting S denote the person is from the south
    ...
    ```{r}
    probability_s <- mean(pop_vs_soda$region == "south")
    ```
...
-   Letting N denote the person is from the northeast
    ...
    ```{r}
    probability_ne <- mean(pop_vs_soda$region == "northeast")
    ```
...
-   Letting W denote the person is from the west
    ...
    ```{r}
    probability_w <- mean(pop_vs_soda$region == "west")
    ```
...
-   Letting M denote the person is from the midwest
    ...
    ```{r}
    probability_mw <- mean(pop_vs_soda$region == "midwest")
    ```
...
-   Determine regional likelihoods that a person uses the word "pop,"
    ...
    ```{r}
    says_pop <- mean(pop_vs_soda$pop == TRUE)
    print(says_pop)
    ```
...
```{r}
(says_pop * probability_s) / says_pop
```
...
```{r}
ggplot(pop_vs_soda, aes(x = region, fill = pop)) + geom_bar()
```
...
4\) Finding Posterior Probability P(S\|A)
